...
from puzzle import Path


class NeuralNetworkViz(QtWidgets.QWidget):
    def __init__(self, parent, puzzle: Path):
        super().__init__(parent)
        self.puzzle = puzzle
        self.horizontal_distance_between_layers = 50
        self.vertical_distance_between_nodes = 10
        self.num_neurons_in_largest_layer = max(self.puzzle.network.layer_nodes)
    ...
    def show_network(self, painter: QtGui.QPainter):
        ...
        layer_nodes = self.puzzle.network.layer_nodes

        default_offset = 30
        h_offset = default_offset
        inputs = self.puzzle.input_values_as_array
        out = self.puzzle.network.feed_forward(inputs)  # @TODO: shouldnt need this
        ...
        for layer, num_nodes in enumerate(layer_nodes):
            ...
            activations = None
            if layer > 0:
                activations = self.puzzle.network.params['A' + str(layer)]
            ...
            for node in range(num_nodes):
                ...
                # Hidden layers
                    ...
                    try:
                        ...
                        print(self.puzzle.network.params)
                        import sys
                        sys.exit(-1)
                    painter.setBrush(QtGui.QBrush(QtGui.QColor.fromHslF(125/239, saturation, 120/240)))
                # Output layer
                elif layer == len(layer_nodes) - 1:
                    text = ('Value ' + str(self.puzzle.network.out[0][0]), 'Place ' + str(self.puzzle.network.out[1][0]), 'L', 'R')[node]
        ...
        for l in range(1, len(layer_nodes)):
            weights = self.puzzle.network.params['W' + str(l)]
